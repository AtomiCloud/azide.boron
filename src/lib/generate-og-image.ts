...
import { OgImageTemplate, type OgImageProps } from './og-template-simple';
...
function loadLogo(): string {
  try {
    ...
    // Return empty string if logo not found - template will use text fallback
    return '';
  }
}
...
/**
 * Generate an Open Graph image using Satori and Sharp
 * Returns a PNG buffer suitable for serving as an image response
 */
export async function generateOgImage(options: GenerateOgImageOptions): Promise<Buffer> {
  ...
  try {
    ...
    const [interRegular, interBold] = await Promise.all([loadGoogleFont('Inter', 400), loadGoogleFont('Inter', 800)]);
